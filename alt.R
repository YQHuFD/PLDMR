    ...
    for (k in 1:length(type)) {
      for (l in 1:length(sa)) {
        ...
        res=do.call(rbind,parLapply(cl,1:r,function(m){
          ...
          y=modified.g%*%alpha+x*beta[l]+e[,2]
          ...
          res_PLDMR=PLDMR(g,x,y,c(type[k],beta[l],sa[l]^2/sigma_y^2,sigma_y^2))
        ...
        write.table(res,paste(c('simu_beta',beta[l],'a',type[k],'sa',sa[l],'LD',rho_g[j],'n',n[i],'.txt'),collapse=''),sep='\t',row.names=F,quote=F)
        stopCluster(cl)
      }
    }
  }
